...
import { DollarSign, ShoppingBag, Clock, TrendingUp, CreditCard, Package, Calendar, Search, RefreshCw, X, ChevronDown } from 'lucide-react';
...
const Analytics: React.FC = () => {
  ...
  const fetchData = (overrideStart?: string, overrideEnd?: string) => {
    setLoading(true);
  ...
  const KPICard = ({ title, value, icon: Icon, color, trend }: any) => (
    <div className="bg-white rounded-xl p-6 shadow-sm border border-gray-200 flex flex-col justify-between hover:shadow-md transition-shadow">
      ...
      {/* Only render trend if explicitly provided */}
      {trend && (
        <div className="flex items-center text-xs">
          <span className="text-green-500 font-bold bg-green-50 px-1.5 py-0.5 rounded flex items-center">
            <TrendingUp className="w-3 h-3 mr-1" /> {trend}
          </span>
          <span className="text-gray-400 mx-2">vs prev. period</span>
        </div>
      )}
    </div>
  );
             ...
             <div className="relative w-full md:w-auto">
                ...
                <input 
                  ...
                  onChange={(e) => {
                    ...
                    setSelectedPreset(''); // Clear preset if manual input
                  }}
                />
             </div>
