...
import { Search, Filter, Calendar, X, FileText } from 'lucide-react';
...
const Orders: React.FC = () => {
  ...
  useEffect(() => {
    // Load all orders
    const all = api.getOrders().sort((a, b) => new Date(b.created_at).getTime() - new Date(a.created_at).getTime());
    setOrders(all);
  }, []);
  ...
  const getStatusBadge = (status: OrderStatus) => {
    const styles = {
      ...
      [OrderStatus.ACCEPTED]: 'bg-blue-100 text-blue-800 border border-blue-200',
      [OrderStatus.IN_KITCHEN]: 'bg-purple-100 text-purple-800 border border-purple-200',
      [OrderStatus.OUT_FOR_DELIVERY]: 'bg-indigo-100 text-indigo-800 border border-indigo-200',
      [OrderStatus.DONE]: 'bg-green-100 text-green-800 border border-green-200',
      [OrderStatus.CANCELLED]: 'bg-red-100 text-red-800 border border-red-200',
    };
    
    const labels: Record<string, string> = {
      [OrderStatus.PENDING]: t('status.pending'),
      [OrderStatus.ACCEPTED]: t('status.accepted'),
      [OrderStatus.IN_KITCHEN]: t('status.in_kitchen'),
      [OrderStatus.OUT_FOR_DELIVERY]: t('status.out_for_delivery'),
      [OrderStatus.DONE]: t('status.done'),
      [OrderStatus.CANCELLED]: t('status.cancelled'),
    };

    return (
      <span className={`px-3 py-1 rounded-full text-xs font-bold ${styles[status] || 'bg-gray-100 border-gray-200'}`}>
        {labels[status]}
      </span>
    );
  };
          ...
          <div className="flex flex-col md:flex-row gap-3">
            ...
            <div className="relative min-w-[160px]">
              ...
              >
                ...
                <option value={OrderStatus.DONE}>{t('status.done')}</option>
